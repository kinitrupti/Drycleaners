...
from drycleaners.models import Customer, Invoice, InvoiceItem, Expense, InvoiceAttachment, ExpenseAttachment
...
def search_invoice(request):
    id = request.POST['id']
    return HttpResponseRedirect(reverse('drycleaners:invoice', args=(id,)))
...
def new_invoice(request):
	...
	if request.method=='POST':
		...
		if customer_id=='None':
			...
			i.save()
			return HttpResponseRedirect(reverse('drycleaners:invoice', args=(i.id,)))
...
def delete_invoice(request, invoice_id):
    invoice = get_object_or_404(Invoice, pk=invoice_id)
    invoice.delete()
    return HttpResponseRedirect(reverse('drycleaners:index'))
...
def upload_invoice_attachment(request, invoice_id):
    ...
    return HttpResponseRedirect(reverse('drycleaners:invoice', args=(invoice.id,)))
